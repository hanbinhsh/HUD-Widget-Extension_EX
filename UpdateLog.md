...
0.9.0新增点击涟漪效果；新增番茄时钟、控制器（滑动条、旋钮和开关）、步进器；修复：涟漪效果面板打开报错、鼠标点击穿透等。<br>
0.9.1完成控制按钮的EXL组件功能，修复EXL报错
0.9.1.1新增steam数据数据源，填入steamID可以获取相关信息
0.9.1.2修复了雷达图调整轴数量不实时更新的问题
0.9.1.3将Steam数据源声明为单例，以免IP被封
0.9.1.4修复控制器和番茄钟在被添加到启动器时的报错;修复了EXL启动器元素不响应悬停和按下颜色、动作的问题;

已知问题：
1. HUD内的控制器控制EXL组件属性后，不会保存
2. 在EXL内设置旋转、透明度等动画会报错widget找不到
3. 如果桌面上没有音频组件，那么HUD内的音频组件不会生效，需要手动修改目前能够快速解决的方案只有去sao根目录/top.mashiros.widget.advp/qml/Common.qml的第170行active:false和174行的wsocket.active = widgetNum>0两行，第一个false和等号后面的都改为true
